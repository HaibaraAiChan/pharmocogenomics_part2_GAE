...
from utils_func import load_data, mask_test_edges, preprocess_graph, get_roc_score
...
if __name__ == '__main__':
    ...
    # Training
        ...
        #     local_batch = local_batch.cuda()
            ...
            assert (len(adj_bs) == len(features_bs))
            for i in range(len(adj_bs)):
                adj = adj_bs[i]
                features = features_bs[i]

                n_nodes = np.array(features).shape[0]
                feat_dim = 1
                adj_norm = preprocess_graph(adj)
                ...
                hidden_emb = mu.data.numpy()
                roc_curr, ap_curr = get_roc_score(hidden_emb, adj)
            #     print("Epoch:", '%04d' % (epoch + 1), "train_loss=", "{:.5f}".format(cur_loss),
            #           "val_ap=", "{:.5f}".format(ap_curr),
            #           "time=", "{:.5f}".format(time.time() - t)
            #           )
            ...
            # print("Optimization Finished!")
            #
            # roc_score, ap_score = get_roc_score(hidden_emb, adj, test_edges, test_edges_false)
            # print('Test ROC score: ' + str(roc_score))
            # print('Test AP score: ' + str(ap_score))
