...
#
# def shortest_path_4_one_node_1(start_node, df, out_folder):
...
def worker(path_list, idx, start_node, df, g, out_folder):
    sum_path_list = []
    vs = VertexSeq(g)
    num = 0
    for path in path_list:
...
def shortest_path_4_one_node(start_node, df, g, out_folder):
    node_idx = g.vs.select(name_eq=start_node)
    if node_idx.__len__() == 0:
    ...
    if size % P_NUM != 0:
        ...
        worker(tail_list, -1, start_node, df, g, out_folder)
        print('tail end')
    ...
    #     p.apply_async(worker, args=(tmp_list, i, start_node, df, g, out_folder))
    #     p.apply_async(worker, args=(sh_path[size//P_NUM*i:size//P_NUM*(i+1)], i, start_node, df, g, out_folder))
    [p.apply_async(worker, args=(sh_path[size // P_NUM * i:size // P_NUM * (i + 1)], i, start_node, df, g, out_folder)) for i in range(P_NUM)]
    p.close()
    p.join()
    # tmp = main_list[0].get()
...
if __name__ == '__main__':
    ...
    [shortest_path_4_one_node(start_node, df, g, output_folder) for start_node in protein_list]
    print('end of all')
