...
class DataGenerator(data.Dataset):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        #     print(str(len(list_IDs_temp))+' samples in the tail')
        ...
        adj_array = np.zeros((self.batch_size*self.dim, self.batch_size*self.dim), dtype=float)
        feature_array = np.zeros((self.batch_size * self.dim, self.batch_size), dtype=float)
        if len(list_IDs_temp) < self.batch_size:
            adj_array[:tmp_adj_array.shape[0], :tmp_adj_array.shape[1]] = tmp_adj_array
            feature_array[:tmp_feature_array.shape[0], :tmp_feature_array.shape[1]] = tmp_feature_array
        else:
