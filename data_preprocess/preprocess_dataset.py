...
if __name__ == "__main__":
    ...
    names = ['interactions', 'gene', 'cellline']
    objects = []
    folder_files = os.listdir(input_folder)
    interaction_file = [f for f in folder_files if names[0] in f][0]
    gene_file = [f for f in folder_files if names[1] in f][0]
    cellline_file = [f for f in folder_files if names[2] in f][0]
    interaction_data = pd.read_csv(input_folder + interaction_file, delimiter='\t')
    gen_data = pd.read_csv(input_folder + gene_file, delimiter=',')
    cellline_data = pd.read_csv(input_folder + cellline_file, delimiter=',')
